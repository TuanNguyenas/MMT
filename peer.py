...
class Peer():
    def __init__(self, ip, port:int = 4040, peer_list:set = set(), header_length = 1024,pieces_storage="pieces", metainfo_storage ="metainfo", output_storage = "output",tracker_ip = "",tracker_port =5050 ) -> None:
    ...
    def join_network(self):
        ...
        try:
            ...
            # Kiểm tra nếu response là None hoặc có lỗi
            if response is None:
                print("No response received from tracker.")
                return
            if response.get("error"):
                print("Error received from tracker:", response.get("error"))
                return
            
            print("Joined network successfully:", response)
            self.id = response.get("id")
            # print("[ID: ]", response.get("id"))
            print("Response from tracker:", response)
        except ConnectionResetError:
            print("[ERROR] Connection was reset by peer.")
    ...
    def recieve_message(self, connection, address= None) -> dict:
        ...
        Returns:
            dict: message to receive.
        """
        try:
            # Đọc độ dài tin nhắn đầu tiên (được gửi trước nội dung)
            message_length = connection.recv(self.header_length).decode("utf-8")
            if not message_length:
                return None  # Nếu không nhận được độ dài, có thể kết nối bị đóng.
            message_length = int(message_length)  # Chuyển độ dài thành số nguyên.
            # print("[DATA LEN]: ", message_length)
            # Đọc dữ liệu theo độ dài xác định.
            message = b''
            while len(message) < message_length:
                data = connection.recv(min(message_length - len(message), 1024))  # Điều chỉnh kích thước bộ đệm.
                # print("[DATA]: ", data)
                if not data:
                    return None  # Xử lý khi kết nối bị ngắt đột ngột.
                message += data
            ...
            print("[ERROR] Connection was reset by peer.")
            return None  # Trả về None hoặc xử lý theo ý muốn.
        
        except Exception as e:
            print(f"[ERROR] Unexpected error: {str(e)}")
            return None  # Trả về None hoặc xử lý lỗi khác.
...
def upload_peer_test():
    # print("[START UPLOAD]")
    # print("START UPLOAD 1")
    peer = Peer(
        # id=1,
        ip="10.0.182.57.",  # bổ sung giá trị 'ip'
        port=4043,
        pieces_storage="pieces1",
        metainfo_storage="metainfo1",
        output_storage="output1",  # bổ sung 'output_storage' nếu cần
        header_length=1024,  # bổ sung 'header_length' nếu cần
        tracker_ip="localhost",
        tracker_port=5050
    )
    # print("START UPLOAD 1")
    # list_up = ["input/meeting_1.mp4", "input/walking.mp4", "input/test.mp4"]
    list_up = ["input/4.3_Usability_Testing.pdf"]
    # print("START UPLOAD 1")
...
def download_peer_test():
    peer = Peer(
        # id=2,
        ip="localhost",  # bổ sung giá trị 'ip'
        port=4042,
        pieces_storage="pieces2",
        metainfo_storage="metainfo2",
        output_storage="output2",  # bổ sung 'output_storage' nếu cần
        header_length=1024,  # bổ sung 'header_length' nếu cần
        tracker_ip="localhost",
        tracker_port=5050
    )
...
# python3 peer.py --ip 10.0.187.206 --port 4042 --tracker-ip localhost --tracker-port 5050  --header-length 1024\
